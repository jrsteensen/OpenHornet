...
| `--alpha ALPHA` | Ink layer opacity, `(0.0, 1.0]` — exclusive lower bound (default: `0.6`) |
...
5. The resulting images are assembled back into a single PDF using lossless PNG embedding for maximum fidelity.
...
The companion workflow `.github/workflows/pdf-overlay-compare.yml` runs automatically on every pull request that includes **any** PDF addition or deletion.  It:

1. Detects added/deleted PDFs using the GitHub API.
2. **Always posts or updates a PR comment** when any PDF change (addition *or* deletion) is detected — even if no comparison can be generated.
...
5. Runs this script for each unambiguous added/deleted pair, generating a lossless overlay PDF per pair.
6. Uploads generated comparison PDFs as a workflow artifact when at least one comparison is produced.

> **Note:** Overlay generation requires at least one unambiguous add+delete pair (i.e. both an old and a new version of the same drawing present in the PR).  If only additions *or* only deletions are present, the workflow will still comment on the PR to report what was detected — it simply won't produce a comparison artifact.
