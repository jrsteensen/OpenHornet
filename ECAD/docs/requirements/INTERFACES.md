...
Observed sources: [architecture inventory](../reference/ARCHITECTURE_INVENTORY.md), [board inventory](../reference/BOARD_INVENTORY.md), [interconnect root](../../interconnects/OH_Interconnect.kicad_sch), and [OH-SPEC-002 revision A](../../../release/OH-SPEC_Specifications/OH-SPEC-002%20-%20ABSIS%20CABLES_A_v2.pdf).
...
## Proposed shield arrangement — Q-03 remains open

Owner intent requires a shielded stranded twisted A/B pair and cable removal without loosening a screw. The current Master has J6, a drain terminal connected to board GND. This proposal uses that existing connection; it does not add protective-earth or chassis bonding.

Propose one short, secured lead from J6 to an insulated splice with a separate detachable drain pigtail for each J2/J3/J4 cable. Keep the six-pin bus connectors unchanged. Each cable disconnects at its bus connector and its drain connector; the J6 screw stays secured. Do not put multiple loose drains under a terminal unless its specification allows that termination.

Prefer a small latched Mini-Fit Jr. inline connection using the existing connector family and tooling. A two-position version can carry drain on one contact, leaving the second unused, but it must have verified mechanical polarization/keying preventing mating to OH power connectors. Color/labels or simply leaving a cavity empty are not mechanical keying. Exact housing/contact/key selection remains part of harness design; if suitable keying cannot be achieved, use a mechanically distinct housing within the family instead of an interchangeable power plug.

Maintain shield continuity along each complete segment through insulated harness-side drain connections at intermediate boards/bulkheads. Bond to board GND only at the owning Master J6; insulate the far end and avoid slave-GND/chassis bonds. Keep the normal bus GND conductor: the shield is not a load return. Keep drain pigtails short and strain-relieved. A later EMC-driven change to chassis bonding needs separate review; single-end drain bonding is a practical proposal for this household installation, not a universal RF shielding rule or hot-plug guarantee.

Validate the final harness for drain continuity, absence of unintended intermediate bonds, correct pin mapping, mechanical misconnection prevention and ordinary communication with representative switched loads. The proposal is not yet an approved cable build instruction.
