...
- ChatGPT Desktop for Linux 26.908.40401-1 successfully exercised as a graphical Codex/Astra front end after its generated bundled-plugin cache was rebuilt; the bundled app-server reported 0.154.0-alpha.6.2.
...
ChatGPT Desktop MAY be used as the graphical Codex/Astra control surface when the same smoke test passes. Open the canonical OpenHornet checkout in **Local** mode, not a Codex-managed worktree, unless the OpenHornet KiCad library variables have first been deliberately repointed and verified for that worktree.

The desktop application and standalone CLI may use different bundled Codex builds even when they share `~/.codex/config.toml`. Record the relevant desktop application/app-server version when it is used for design work. Confirm Konnect reaches `ready` and perform the same read-only board smoke test before mutation.

Project-local ChatGPT Desktop environment files under `.codex/environments/` are autogenerated, machine-local state in the current reference setup and are intentionally ignored by Git. Desktop environment **Actions are optional convenience UI**, not part of the required OpenHornet workflow. The repo-owned `utils/tools/ecad/` preflight/launch utilities are authoritative; GUI users can run the appropriate preflight from the integrated terminal before an Astra task.

If the Linux desktop application crashes before project work, distinguish application state from the OpenHornet toolchain before modifying repository or MCP configuration. In the validated setup, stale generated bundled-plugin cache state could crash the UI while Konnect itself was healthy; rebuilding only generated application plugin/cache state restored the GUI without changing `~/.codex/config.toml`, Konnect, or OpenHornet.
...
On the validated Fedora 44 KDE/Wayland workstation, Konnect 0.11.1's viewer required XWayland plus the WebKitGTK DMABUF workaround:

```bash
GDK_BACKEND=x11 WEBKIT_DISABLE_DMABUF_RENDERER=1 schematic-viewer /path/to/root_schematic.kicad_sch
```

A local wrapper MAY contain those two environment variables so they affect only the viewer. The current viewer's dark schematic theme applies a CSS filter to the SVG and rendered visibly soft under this WebKitGTK path at high zoom; the **light theme retained sharp vector rendering** and is the validated review mode. These are workstation/viewer compatibility notes, not schematic requirements.
