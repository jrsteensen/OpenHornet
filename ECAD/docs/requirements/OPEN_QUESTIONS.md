...
Resolve Q-01 through Q-04 before using an ambiguous design as the next prototype baseline. Identify owners and board-specific decisions for remaining open electrical/qualification items as needed. Exercise the adopted toolchain and review record on each scoped hardware change, then expand validated patterns deliberately rather than inferring blanket standards from historical boards. Hardware corrections should be separately scoped PRs with their own evidence.
