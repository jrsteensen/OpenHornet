...
Apply QUAL-008/010 to compare the final 8.0.1 source and outputs with the exact tested v8 baseline. Human review may carry forward relevant physical evidence when the final delta is limited to formatting/silkscreen and does not alter electrical, mechanical, assembly or functional behavior. Check version identity, pin/polarity legends, component visibility and output regeneration; no automatic full redesign or repeat prototype is required solely for a justified cosmetic revision. Any significant electrical/layout change invokes QUAL-009.
