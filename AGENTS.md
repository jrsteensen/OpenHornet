...
- Use [the review record](ECAD/docs/requirements/REVIEW_RECORD.md) for decisions, exact toolchain versions, evidence, exceptions and qualification status. Record what was checked, what was physically tried, the outcome and any limitations; a short PR report is sufficient.
- AI-produced ECAD requires ordinary maintainer review. Successful existing builder use is acceptable evidence absent known relevant problems. New boards and significant revisions need an actual build and relevant operating checks before being called tested; a builder installation can supply that evidence. No separate engineering-signoff or formal qualification dossier is required. ERC/DRC alone does not establish function.

- Prefer single-sided SMD assembly; double-sided assembly is allowed when justified despite its higher cost. Use black soldermask and white silkscreen.
- Keep enduring policy in requirements and actual defects/work items in repository issues/PRs. Do not create issues without task authorization.
- Update a board's current files in place for new revisions; Git preserves history. Obsolete designs may remain temporarily and be removed through ordinary review without deleting Git history or existing release artifacts.

The framework adopted by merged PR #1255, as amended by owner decisions, applies to new designs and changed portions without retroactive blanket redesign. Repository-authorized mergers may merge development hardware before physical qualification after review with disclosed status/provisional outputs. Merge does not qualify hardware. Only John Steensen may explicitly accept a very minor change as fully qualified without a new prototype under QUAL-010.
